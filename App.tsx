  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message.includes("Requested entity was not found") || err.message === "MISSING_KEY") {
        ...
        setError("Analysis failed. Please ensure you have a valid API key with a paid billing project linked.");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
          ...
          <div className="flex items-center gap-4">
            {isAuthorized ? (
              <div className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-100 rounded-full text-[10px] font-black text-green-700 uppercase tracking-widest">
                <ShieldCheck size={14} /> Key Linked
              </div>
            ) : (
              <button 
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 flex-grow w-full">
        ...
        {error && (
          <div className="max-w-2xl mx-auto mb-10 bg-red-50 border border-red-200 p-6 rounded-3xl text-red-700 font-bold flex items-center gap-4">
            <AlertCircle /> {error}
          </div>
        )}
